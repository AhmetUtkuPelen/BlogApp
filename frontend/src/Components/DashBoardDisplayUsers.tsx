    ...
    const FetchUsers = async () => {
      try {
        ...
        const response = await axios.get(
          `${import.meta.env.VITE_SERVER_URL}/api/users/getAllUsers`
        );
  ...
  const HandleShowMorePosts = async () => {
    ...
    try {
      const response = await axios.get(
        ...
        }/api/users/getAllUsers?startIndex=${startIndex}`
      );
      const data = response.data;
      if (response.status === 200) {
  ...
  const HandleDeleteUser = async () => {
    ...
    try {
      const response = await axios.delete(
        `${
          import.meta.env.VITE_SERVER_URL
        }/api/users/adminDeleteUsers/${userIdToDelete}`
      );
      const data = response.data;
      if (response.status === 200) {
        setUsers((prevUsers) =>
                ...
                <Table.Row>
                  ...
                  <Table.Cell>
                    {user?.isAdmin ? (
                      ...
                      <LuUserRoundX className="text-red-500 text-3xl" />
                    )}
                  </Table.Cell>
                  <Table.Cell>
                    <MdDeleteForever
