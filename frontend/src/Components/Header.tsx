  ...
  return (
    <Navbar className="border-b-2">
      ...
      <div className="flex gap-2 md:order-2">
        ...
        {currentUser ? (
          <Dropdown className="text-center" arrowIcon={false} inline label={<Avatar alt="user image" img={currentUser?.currentUser?.profilePicture} rounded/>}>
            <Dropdown.Header>
              {currentUser?.currentUser && (
                <>
                  <span className="block text-md text-blue-600 font-bold">{currentUser.currentUser.username}</span>
                  <br/>
                  <span className="block text-md truncate text-gray-500">{currentUser.currentUser.email}</span>
                </>
              )}
            </Dropdown.Header>
            ...
            {currentUser?.currentUser ? (
              ...
              <>
                ...
                <Dropdown.Divider />
                <Dropdown.Item className="text-red-600 text-center justify-center" onClick={UserHandleLogout}>
                  LOG OUT
                </Dropdown.Item>
              </>
            ) : (
