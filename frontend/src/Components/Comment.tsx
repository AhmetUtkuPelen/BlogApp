...
const Comment = ({ comment, onLike, onEdit, onDelete }: CommentProps) => {
    ...
    const getUser = async () => {
      try {
        const response = await axios.get(
          `${import.meta.env.VITE_SERVER_URL}/api/users/getUserById/${
            comment.userId
          }`
        );
  ...
  const handleUpdateComment = async () => {
    try {
      await axios.put(
        `${import.meta.env.VITE_SERVER_URL}/api/users/updateComment/${
          comment._id
        }`,
        {
  ...
  return (
    <div>
      ...
      <div className="p-4 border-b border-gray-200 mt-2 flex-1">
        <div className="">
          <span className="font-bold mr-1 text-sm truncate text-ellipsis text-teal-500">
