    ...
    const FetchPosts = async () => {
      try {
        ...
        const response = await axios.get(
          `${import.meta.env.VITE_SERVER_URL}/api/users/getAllPosts?userId=${
            currentUser?._id
          }`
        );
  ...
  const HandleDeletePost = async () => {
    ...
    try {
      const response = await axios.delete(
        ...
        }/api/users/deletePost/${postIdToDelete}/${currentUser?._id}`
      );
      const data = response.data;
      if (response.status !== 200) {
        console.log(data);
        ...
        <>
          ...
          >
            <Table.Head className="border-b-2">
              ...
              <Table.HeadCell className="text-center text-green-500">
                Edit
              </Table.HeadCell>
            </Table.Head>
            ...
            {posts.map((post) => (
              <Table.Body key={post._id}>
                <Table.Row>
                  ...
                  <Table.Cell className="transition-transform duration-300 hover:scale-125">
                  ...
                  <Table.Cell>
                    <Link to={`/updatePost/${post._id}`}>
                      <FaEdit className="text-blue-500 hover:text-blue-600 cursor-pointer text-3xl" />
                    </Link>
                  </Table.Cell>
                </Table.Row>
              </Table.Body>
