        ...
        <>
          <Table hoverable className="shadow-md text-center min-w-[400px] max-w-[1000px]">
            <Table.Head className="border-b-2">
              ...
              <Table.HeadCell className="text-center text-blue-500">Comment</Table.HeadCell>
              <Table.HeadCell className="text-center text-blue-500">Number Of Likes</Table.HeadCell>
              <Table.HeadCell className="text-center text-blue-500">PostId</Table.HeadCell>
              <Table.HeadCell className="text-center text-blue-500">UserId</Table.HeadCell>
            ...
            {comments.map((comment) => (
              <Table.Body key={comment._id}>
                <Table.Row>
                  <Table.Cell>{comment.updatedAt && new Date(comment.updatedAt).toLocaleDateString()}</Table.Cell>
                  <Table.Cell className="uppercase transition-transform duration-300 hover:scale-110 hover:underline hover:text-blue-500">{comment.content}</Table.Cell>
                  <Table.Cell className="uppercase">{comment.numberOfLikes}</Table.Cell>
                  <Table.Cell className="uppercase">{comment.postId}</Table.Cell>
                  <Table.Cell className="uppercase">{comment.userId}</Table.Cell>
                  <Table.Cell><span onClick={() => {
                    setShowModal(true);
                    if (comment._id) setCommentIdToDelete(comment._id);
                  }} className="text-red-500 hover:text-red-600 cursor-pointer text-4xl"><MdDeleteForever/></span></Table.Cell>
                </Table.Row>
              </Table.Body>
            ))}
            ...
            {showMoreComments && (
              <button onClick={HandleShowMoreComments} className="bg-blue-500 text-white px-4 py-2 rounded-md w-full self-center mt-4">MORE COMMENTS</button>
            )}
