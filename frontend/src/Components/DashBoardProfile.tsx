...
import { deleteUserStart, updateFailure, updateStart, updateSuccess, deleteUserSuccess , deleteUserFailure} from "../../Redux/Slices/UserSlice.ts";
...
const DashBoardProfile = () => {
  ...
  const uploadImage = async (image: File) => {
    ...
    // ? Add userID to path ? \\
    const storageRef = ref(storage, `profilePictures/${currentUser?._id}/${fileName}`)
...
const UserHandleDelete = async () => {
  ...
  try {
    ...
    dispatch(deleteUserFailure(error instanceof Error ? error.message : 'Something went wrong'));
  ...
  return (
    <div className='max-w-screen mx-auto p-3 w-full'>
      <h1 className={`text-center font-semibold text-3xl`}>PROFILE</h1>
        <form className='flex flex-col cursor-pointer mt-4 gap-4' onSubmit={handleSubmit}>

            <input type='file' accept='image/*' onChange={handleImageChange} ref={filePickerRef} hidden/>

            <div className='w-32 h-32 self-center' onClick={() => filePickerRef.current?.click()}>
              {}
                <img src={imageFileUrl || currentUser?.profilePicture} alt={`Profile-Picture`} className='rounded-full h-full w-full border-8 border-[lightgray] object-cover'/>
            </div>
            ...
            <Button type='submit' color='blue' outline>UPDATE</Button>

            {currentUser?.isAdmin && (
              <Link to='/createPost'><Button type='button' className="mt-4 w-full" color='blue'>CREATE POST</Button></Link>
            )}
            ...
            <Modal.Header/>
              <Modal.Body>
                <div className="text-center">
                  ...
                  <h3 className="mb-5 text-center text-lg text-blue-500">? You Sure You Want To Delete Your Account ?</h3>
                  <div className="flex justify-center gap-4">
                    <Button color="failure" onClick={UserHandleDelete}>DELETE</Button>
